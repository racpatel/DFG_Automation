...
testname=hybrid-48hr-site1-$comment
...
echo `date +%y/%m/%d" "%H:%M` " hybrid-48hr ..."
./runIOworkload-site1.sh generic-hybridSS-48hr.xml
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 hybrid-48hr (${jobId}) complete" twilkins@redhat.com
fi
