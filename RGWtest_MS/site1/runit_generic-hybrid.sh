...
echo `date +%y/%m/%d" "%H:%M` " hybrid ..."
./runIOworkload-site1.sh generic-hybridSS.xml
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 hybrid (${jobId}) complete" twilkins@redhat.com
fi
