...
testname=small-hybrid-site1-$comment
...
echo `date +%y/%m/%d" "%H:%M` " small-bybrid ..."
./runIOworkload-site1.sh small-hybridSS.xml
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 small-hybrid (${jobId}) complete" twilkins@redhat.com
fi
