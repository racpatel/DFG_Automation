...
./Utils/cos.sh generic-fillWorkload-init.xml RESULTS/init_`date +%y%m%d-%H%M`.log
...
# disable postPolling for fill job
sed -i -s 's/postPolling=true/postPolling=false/' vars.shinc

echo `date +%y/%m/%d" "%H:%M` " fill-prep ..."
./runIOworkload-site1.sh generic-fillWorkload-prep.xml
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 fill (${jobId}) complete" twilkins@redhat.com
fi
