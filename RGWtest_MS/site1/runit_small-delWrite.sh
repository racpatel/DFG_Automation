...
testname=small-delWrite-site1-$comment
...
ansible all -i ${nodelist} -m shell -a "nohup /usr/local/bin/vmstat.sh -i 20 > /tmp/vmstat.txt &" > /dev/null

echo `date +%y/%m/%d" "%H:%M` " small-delWrite ..."
./runIOworkload-site1.sh small-deleteWrite2hr.xml
...
if [[ $bsV2Polling == "true" ]]; then
    ...
    for i in $(cd /rootfs/root/RGWtest/RESULTS/ && ls osd.*.log) ; do mv ${basepath}/RESULTS/$i $outdir/bsv2perf/${jobId}_${i} ; done
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 small-delWrite (${jobId}) complete" twilkins@redhat.com
fi
