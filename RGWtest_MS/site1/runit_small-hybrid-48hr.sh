...
testname=small-hybrid48hr-site1-$comment
...
echo `date +%y/%m/%d" "%H:%M` " small-hybrid-48hr ..."
./runIOworkload-site1.sh small-hybridSS-48hr.xml
...
if [[ $cephadmshell != "true" ]]; then
    echo " " | mail -s "RGWtest site1 small-hybrid-48hr (${jobId}) complete" twilkins@redhat.com
fi
