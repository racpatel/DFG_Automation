...
./Utils/cos.sh fillWorkload-init.xml RESULTS/init_`date +%y%m%d-%H%M`.log
...
./runIOworkload-site2.sh fillWorkload-prep.xml
...
#if [[ $cephadmshell != "true" ]]; then
#    echo " " | mail -s "RGWtest site2 fill (${jobId}) complete" <email.address>
#fi
