...
testname=bb-fill-site2-$comment
...
./Utils/cos.sh bb-fillWorkload-init.xml RESULTS/bb-init_`date +%y%m%d-%H%M`.log
...
echo `date +%y/%m/%d" "%H:%M` " bb-fill-prep ..."
./runIOworkload-site2.sh bb-fillWorkload-prep.xml
...
#if [[ $cephadmshell != "true" ]]; then
#    echo " " | mail -s "RGWtest site2 bb-fill (${jobId}) complete" <email.address>
#fi
